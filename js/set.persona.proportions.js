...
function dispatchPERSONA (stepdata) {
	if (stepdata.from) {
		if (stepdata.type == 'proportion') return drawPERSONAPROPORTION(stepdata);
		if (stepdata.type == 'absolute') return drawPERSONA(stepdata);
	} else {
		return drawPERSONA(stepdata);
	}
}

function drawPERSONAPROPORTION (stepdata) {
	var svg = d3.select('#svg-container > svg'),
	...
	setTimeout(function () {
		...
		if (stepdata.units.l == 'grid') {
			...
			if (['bar', 'line', 'area'].indexOf(stepdata.from) !== -1) {
				...
				var bbox = slice.node().getBBox(),
					centroid = [bbox.x + bbox.width / 2, bbox.y + bbox.height / 2];
				var afactor = 0, bfactor = 0;
				if (centroid[0] < 0) bfactor = -1;
				if (centroid[0] > 0) afactor = -1;
				...
				unit.transition()
					...
					.attr('transform', function (d) {
						...
						if (d.type == 'below' && bfactor == -1) return 'translate(' + [(bfactor * __svg__.w / 2) + (__svg__.w / 2 - maxline * xshapesize) / 2 + xshapesize / 2, 0] + ')';
						if (d.type == 'below' && bfactor == 0) return 'translate(' + [(__svg__.w / 2 - maxline * xshapesize) / 2 + maxline * xshapesize + xshapesize / 2, 0] + ')';
					});
				...
				unit.transition()
					...
					.delay(timeout)
					.attr('transform', function (d) {
						if (d.type == 'above') return 'translate(' + [(afactor * __svg__.w / 2) + (__svg__.w / 2 - maxline * xshapesize) / 2 + xshapesize / 2, __svg__.pt / 2 - __svg__.h / 2] + ')';
						
						if (d.type == 'below') return 'translate(' + [(bfactor * __svg__.w / 2) + (__svg__.w / 2 - maxline * xshapesize) / 2 + xshapesize / 2, __svg__.pt / 2 - __svg__.h / 2] + ')';
					});
		...
		if (stepdata.units.l == 'grid') {
			if (['bar', 'line', 'area'].indexOf(stepdata.from) !== -1) {
				...
				unit.transition()
					...
					.delay(timeout)
					.attr('transform', function (d) {
						/*if (d.type == 'above') return 'translate(' + [__svg__.w / 4 - (maxline * xshapesize) / 2 -__svg__.w / 2, __svg__.pt / 2] + ')';
						if (d.type == 'below') return 'translate(' + [__svg__.w * 3 / 4 - (maxline * xshapesize) / 2 - __svg__.w / 2, -__svg__.h / 2 + __svg__.pt / 2] + ')';*/
						if (d.type == 'above') return 'translate(' + [(afactor * __svg__.w / 2) + (__svg__.w / 2 - maxline * xshapesize) / 2 + xshapesize / 2, __svg__.pt / 2 - __svg__.h / 2] + ')';
...
function addUNITS (stepdata, n, datum, delay, color) {
	...
	if (descrbie_units && ['bar', 'line', 'area'].indexOf(stepdata.from) !== 1) { acount = 1; above = 1; bcount = 1; below = 1; }
	if (descrbie_units && stepdata.from == 'pie') { acount = 0; above = 0; }
			...
			if (j < n) {
				if (above == n - (threshold - 1)) {
					...
					if (['bar', 'line', 'area'].indexOf(stepdata.from) !== -1) {
						...
						if (stepdata.units.t == 'bubble') node = { type: type, shape: stepdata.units.t, x: 0, y: 0, fill: slice_datum.fill }; // BECAUSE THE g.chart IS ALREADY TRANSLATED TO THE CENTER
						if (stepdata.units.t == 'isotype') node = { type: type, shape: stepdata.units.t, x: 0, y: -42, fill: slice_datum.fill }; // BECAUSE THE g.chart IS ALREADY TRANSLATED TO THE CENTER
					}
				...
				if (stepdata.units.l == 'grid') {
					if (['bar', 'line', 'area'].indexOf(stepdata.from) !== -1) {
						...
						unit.transition()
							.duration(__animation__.dur / 2)
							.attr('transform', function (d) {
								if (d.type == 'above') return 'translate(' + [(afactor * __svg__.w / 2) + (__svg__.w / 2 - maxline * xshapesize) / 2 + acount * xshapesize + xshapesize / 2, __svg__.pt / 2 + aline * yshapesize - __svg__.h / 2] + ')';
								
								if (d.type == 'below') return 'translate(' + [(bfactor * __svg__.w / 2) + (__svg__.w / 2 - maxline * xshapesize) / 2 + bcount * xshapesize + xshapesize / 2, __svg__.pt / 2 + bline * yshapesize - __svg__.h / 2] + ')';
							});
	...
	// ANIMATE THE BUBBLES IN THE FORCE VIEW TO SEPARATE HORIZONTALY
...
function updateUNITS (stepdata, a, b, t) {
	...
	if (stepdata.from == 'pie') {
		maxline = 12;
		var node = d3.select('.persona.below')
			centroid = d3.transform(node.attr('transform')).translate,
			afactor = 0, bfactor = 0;
		if (centroid[0] < 0) bfactor = -1;
		if (centroid[0] > 0) afactor = -1;
	}
